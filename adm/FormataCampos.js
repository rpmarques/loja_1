...
// Jacson -- 06/02/15 >> Mascara de Moeda, Chamar no Onblur
function blurFloat(campo, evt, dec) {
    ...
    var v = campo.value.replace(er, "");
    var f = parseFloat(v.replace(',', '.'));

    var c = !isNaN(f) ? String(f.toFixed(dec)).replace('.', ',') : "";
...
//Jacson 10/02/15
function blurFloatNeg(campo, evt, dec) {
...
// Jacson -- 06/02/15 >> Mascara de Moeda, Chamar no Onblur
function blurFloatRange(campo, evt, dec, rIni, rFin) {
    ...
    var v = campo.value.replace(er, "");
    var f = parseFloat(v.replace(',', '.'));
    var ini = parseFloat(rIni);
    var fin = parseFloat(rFin);
    ...
    if (f >= ini && f <= fin) {
        ...
        if (f < ini) {
            var c = String(ini.toFixed(dec)).replace('.', ',');
            campo.value = c;
        }
        else {
            var c =  String(fin.toFixed(dec)).replace('.', ',');
            campo.value = c;
        }
    }
...
// Formata o campo valor monetário
function formataValorPress2(campo, evt) {
    //1000000,00
    var xPos = PosicaoCursor(campo);
    evt = getEvent(evt);
    var tecla = getKeyCode(evt);
...
// Formata o campo valor monetário
function formataValorPress4(campo, evt) {
    //100000,0000
    var xPos = PosicaoCursor(campo);
    evt = getEvent(evt);
    var tecla = getKeyCode(evt);
...
function formataValorPress6(campo, evt) {
    //100000,0000
    var xPos;
    ...
    if (vr.length > 0) {
        ...
        if (tam == 0)
            campo.value = "0,00000" + vr;
        if (tam == 1)
            campo.value = "0,0000" + vr;
        if (tam == 2)
            campo.value = "0,000" + vr;
        if (tam == 3)
            campo.value = "0,00" + vr;
...
function mascaraGenerica(campo, evt, Padrao) {
    ...
    //Define dinamicamente o tamanho maximo do campo de acordo com o padrao fornecido  
...
function formataHora(campo, evt) {
    ...
    if (h >= 0 && h <= 24) //valida de a hora é menor que 24 e maior que 0, senão retorna o valor dentro da faixa de horas possíveis -- Jacson 27/03/15
        h = h;
    else if (h < 0)
        h = 00;
    else if (h >= 24)
    ...
    if (m >= 0 && m <= 60) //valida os minutos se são menores que 60 e maiores que 0, senão retorna o valor dentro da faixa de minutos possíveis -- Jacson 27/03/15
        m = m;
    else if (m < 0)
        m = 0;
    else if (m >= 60)
...
function preencheHora(campo, evt) {
...
function formataCodigoEstruturado(campo, evt) {
    ...
    if ((tam >= 4) && (tam < 7))
    { campo.value = vr.substr(0, 1) + '.' + vr.substr(1, 2) + '.' + vr.substr(3, 3); }
    if ((tam >= 7) && (tam < 11))
    { campo.value = vr.substr(0, 1) + '.' + vr.substr(1, 2) + '.' + vr.substr(3, 3) + '.' + vr.substr(6, 4); }
    if (tam >= 11)
    { campo.value = vr.substr(0, 1) + '.' + vr.substr(1, 2) + '.' + vr.substr(3, 3) + '.' + vr.substr(6, 4) + '.' + vr.substr(10, vr.length - 10); }
